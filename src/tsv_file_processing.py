...
def prsm_file_processing(prsm_file_name, proteoform_file_name):

    path = os.path.abspath(prsm_file_name)
    print(path)
    
    # remove comments for prsm
    prefix, ext = os.path.splitext(prsm_file_name)
    prsm_file_remove_comment = prefix + "_remove_comment.tsv"
    rcl.remove_first_29_lines(prsm_file_name, prsm_file_remove_comment)
    
    # remove comments for proteoform
    prefix, ext = os.path.splitext(proteoform_file_name)
    form_file_remove_comment = prefix + "_remove_comment.tsv"
    rcl.remove_first_29_lines(proteoform_file_name, form_file_remove_comment)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python script.py <input_prsm_file_name> <input_proteoform_file_name>")
    else:
